...
type StatusInKeystore struct {
	...
	// If present, indicates when the key will be destroyed in the target keystore, expressed as a duration (e.g. PT24H
	// means 24 hours). If not possible to determine, or does not apply, an empty string is returned.
	DestructionIn *string `json:"destruction_in,omitempty"`
...
func UnmarshalStatusInKeystore(m map[string]json.RawMessage, result interface{}) (err error) {
	...
	err = core.UnmarshalPrimitive(m, "destruction_in", &obj.DestructionIn)
	if err != nil {
		return
	}
